...
def minkowski(xUnits, yUnits, resolution, rad, clc):
	...
	'''
	global workSpace
	workSpace = np.ones((xUnits+1, yUnits+1), dtype=np.float64)*255

	configSpace = np.ones((int(round(xUnits/resolution))+1, (int(round(yUnits/resolution))+1)), dtype=np.float64)*255
...
def main(args):
	...
	# workSpace, configSpace = minkowski(xUnits, yUnits, 1, 22.0, 0)
